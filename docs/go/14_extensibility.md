...
- **Size-catalog amendment (Forgejo #248, R1 B1/B2/B3 — lands first, #247 builds on it).**
  ...
  `repos/<o>/<r>/meta/stats.json` (CAS'd per-repo sidecar, JSON version 1:
  size_bytes + object_count + head_seq + updated_at; #247 activity derivation
  adds optional fields on this SAME file — one PUT, both concerns, never two
  sidecars) and `meta/repos.pb` field-3 `entries` (CAS'd aggregate rows on the
  existing bucket-root key — no new bucket-root family). Proto is append-only
  ...
  establishing the rails activity follows.
