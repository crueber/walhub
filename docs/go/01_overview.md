...
- **Divergence (2026-08-31, D2):** the frontend is standard ECMAScript — no TypeScript, no framework. Vanilla ES modules served directly from `web/` (native import maps, `<template>`, ~40 lines of hand-rolled reactive helpers), zero npm runtime dependencies; the SPA has no build step. The SDK is **authored as submodules** (`web/sdk/src/*.js`) and esbuild-bundled into the shipped `web/dist/repos.js` — the single dev-time build (amended the same day per user direction: "a build step so repos.js can be broken out into submodules"; esbuild is the one devDependency). JS tests run on Node's built-in `node --test` against the sources plus server-smoke tests over fetch. Supersedes SolidJS/vite and the ≤ 6-npm-dependency frontend budget.
