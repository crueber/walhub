...
- **Data** (dogfood rule intact): `useData("owners", …owners.list({sort:"activity",order:"desc"}))` (Forgejo #283 follow-up — the server ranks ALL owners by the per-owner max-commit rollup before the client slices, so first paint is ordered and active owners past the name cap surface), then one `OwnerSection` per
  ...
  `owners.listDetailed()` (07 §8 — per-owner `last_commit_time` rows, same sort query).
