...
`web/src/pages/Landing.jsx`: static marketing, ZERO API calls (no `useData`, no SDK import, no `fetch` — the front door spends no store round trips; pinned by `web/test/unit/landing.test.js`). Hero (H1 + sub in README L3–5 language + two CTAs only: "Browse repositories" → `/explore`, "Push in 30 seconds" → `#quickstart`; setup lives in the top nav + the quickstart auth line, the walgit object-protocol one-liner lives on `/how-it-works` — both removed from the hero by #229), four alternating concept sections (GIF + caption + 2–3 sentences: push, bucket-as-database, fetch/clone, collaboration-as-objects), and a `#quickstart` strip (the README push block + auth-modes one-liner + SSH one-liner + `/explore` CTA + the "How it works →" deep-dive CTA at the page bottom). `*` fallback renders Landing (safe default: no API calls, no confusing empty state).
...
- **Data** (dogfood rule intact): `useData("owners", …owners.listDetailed({sort:"activity",order:"desc"}))` (Forgejo #295 over the #283 follow-up — the server ranks ALL owners by the per-owner max-commit rollup before the client slices, so first paint is ordered and active owners past any cap surface; the rows carry each owner's `last_commit_time`, so the page filters to active owners with zero extra GETs), then one `OwnerSection` per
  shown owner with `useData(`repos:${owner}`, …owners.detailed(owner, {sort:"activity",order:"desc"}))` — the same cache key the
  `/:owner` page uses, so the two pages share fetches. The object-row owners surface is
  `owners.listDetailed()` (07 §8 — per-owner `last_commit_time` rows, same sort query). The intro card's owner total is the payload's uncapped row count — never the page's slice. The intro card's repo total (Forgejo #307) is `instanceRepoTotal` over the SAME uncapped payload rows (the sum of the served `repo_count` fields — never the top-5 slice, never a per-owner listing walk).
- **Order**: owner sections most-recent-commit-first (Forgejo #283 — the
  server orders the names (`sort=activity&order=desc` over the derived
  rollup) AND each section reports `ownerActivity` over the detailed doc it
  already fetches, the page re-ranks via `orderOwnersByActivity` as docs
  land — the client rank is the fallback/enhancement for missing values
  and stale catalog data, never contradicting the server (same total
  order: known desc, unknown/pending last, name tiebreak); unknown/pending
  sorts last, name tiebreak), repos most-recent-commit-first (Forgejo #247). The listing
  ...
  (`sort=activity&order=desc`) and the client stabilizes with
   `orderByActivity` in `web/src/lib/owners.js` (RFC 3339 desc, unknowns last,
    ties on `(owner, name)` — the server's key order — headless-tested).
     (Forgejo #283: owner SECTIONS arrive server-ordered by the rollup and
    re-rank on commit activity via `orderOwnersByActivity` as section docs
    land; `newestFirst` is a retained legacy proxy, unused by the page.)
    (Forgejo #295: the active-only filter (`activeOwnerNames` over the
    detailed rows — null `last_commit_time` dropped) runs BEFORE the top-5
    slice, so settled-inactive owners never mount; the re-rank above applies
    to the shown sections only.)
- **Caps**: `MAX_OWNERS` 5 owner sections (Forgejo #295 — the top 5 most
  active owners, not 50 name-sorted), `MAX_REPOS_PER_OWNER` 10 repos per section (constants in
  `web/src/lib/owners.js`, headless-tested). Owners with no known commit time never mount (fewer than 5 active owners renders fewer sections, no filler); overflow folds behind links, never a spinner: per-owner
   `+N more →` to `/:owner` (uncapped there), and a "showing top 5 of A active owners" line only when active owners overflow the cap. The intro card carries the instance owner total ("Home to N owners"). The page
  costs 1 + shown-owners SWR GETs (6 cold, down from ~51); per-owner sections render independently (one slow owner never
  blocks the rest).
...
- **Empty/degraded short-circuit (issue #209):** step 0 peeks the shell's shared `repo:{full}` summary entry (`peekCached` — review S3's named mechanism; no fetch): known-empty settles the `EMPTY_REPO` sentinel with **zero doomed requests** (the selArgs-null pattern — a suppressed fetch, not a filtered error). Fallback when the summary is not yet loaded: a 404 carrying the server's `empty repository:` prefix maps to the same sentinel (damage-404s carry no marker by design, so degraded-as-empty misclassification is impossible). Sha-step 404s on a known-degraded repo settle `DEGRADED_MISSING` for inline notices (`tolerateDegraded` wrapper: 404 + known-degraded → sentinel, everything else rethrows — never a global 404 mute); the sentinel caches under the sha key, so pages offer a retry (invalidate) and the WAL re-audit button invalidates the `sha:` prefix. Predicates (`isEmptySummary` incl. the unborn-shape fallback for old servers, `isDegradedSummary`, `isEmptyError`) live in headless-testable `lib/data.js`.
...
- **The repo context also shares the summary signal** (issue #209): tab pages read the shell's entry through `ctx.summary` — no second subscription, no extra fetch. `undefined` = loading, `null` = missing/deleted (the #200 `tolerateMissing` shell).
- **Degraded banner** (issue #209): when the summary reports `health:"degraded"`, an amber (`border-amber-500`, both themes) `role="status"` banner renders under the tab bar — "Some objects are missing (fsck: N missing). Reads may fail; pushes of new refs still work. Details in Settings → WAL." (link `/{full}/settings#wal`). Never a toast, never blocks navigation.
...
- **Object-health card** (issue #209): renders the overview `fsck` projection when a cached audit report exists (absent otherwise — never-audited repos show no card): `missing_total`, the bounded missing-oid sample (first 20 + "…and N more"), `problems`, `repaired_seq`, last-audit time, the effective repair source (or "none — set `upstream.git` to enable repair"), the amber `repair_stalled` notice, and a "run fsck now" trigger (the existing `fsck` op — single-flight, SSE attach — followed by a `sha:` prefix invalidation so healed objects refetch past cached degraded sentinels).
...
- **AMENDED (issue #35) — Latest release panel redesign:** the bare "Latest / No published releases." sidebar box becomes a latest-release card (tag link, draft/prerelease badges, name, `time`-stamped publish date, first 3 asset download links with sizes via the headless-testable `keyAssets` helper in `web/src/lib/releases.js`, "+N more" + "view release" links to the detail page; a `loading latest…` status while the 404-means-none fetch settles so the empty state never flashes) and the shared `Empty` callout (`compact` prop + `.empty-state-compact` styles for the 320 px sidebar) when no release is published. Main list untouched; no backend change — `GET …/releases/latest` already returns the full release including assets. (Superseded by issue #270 below: the sidebar is deleted, Latest folds into the list's first row.)
...
- **FIXED (issue #37) — clone dialog redesign:** the bare `git clone <https-url>` code block becomes a protocol-aware popover — an HTTPS/SSH toggle (`role="group"`, `aria-pressed`, shared `.btn` + `.btn-active` styling), a readonly command textbox that auto-copies on click (Clipboard API with an execCommand fallback, `lib/clone.js` `copyText`) plus a keyboard-reachable Copy button, and a transient `role="status"` "copied" indication (2 s, cleared on unmount; "copy failed — press Ctrl+C" when neither path works, textbox left selected). The `<details>`/`<summary>` popover stays the keyboard baseline (Enter/Space toggle, Tab walks controls); Esc closes and returns focus to the trigger. URL logic is unchanged: HTTPS is the server's `clone_url` verbatim, SSH is the server's `ssh_clone_url` verbatim when advertised (external host+port per 17_ssh.md §3) and otherwise reuses the https hostname at the default ssh port (`ssh://git@host/owner/repo.git` — the served `internal/sshd` form per README/`bind_ssh_test.go`/the /keys copy; the HTTP port is never carried over). Setup recipes stay on the HTTPS URL (their bodies are https-only: bundle-URI URLs, `http.extraHeader`). The `.clone-body` panel gets a solid background (the shared `.card` is translucent in dark mode and tab text bled through). Headless cover: `web/test/unit/clone.test.js` (https verbatim/fallback, ssh advertisement verbatim + host reuse + port drop + host fallbacks, clipboard/fallback/failure copy paths). (Amended by issue #215: the server now advertises `ssh_clone_url` — `server.ssh.external_port` else the listen port — and the clone menu plus the empty-repo guide show it verbatim.)
- **FIXED (issue #49) — new-release composer redesign:** the bare boxed form adrift in the page's right whitespace becomes a centered `max-w-2xl` composer (the IssueNew/PullNew convention) — heading row with an "all releases" back link, section fieldsets (Target / Content / Visibility) with per-field help text, the tag datalist picker kept (fed by the existing `tags({n: 100})` stream; superseded by Forgejo #254 below — now a styled combobox) plus a tag-count/empty/loading line under it, title placeholder defaulting to the typed tag, draft/prerelease as bordered checkbox option rows with one-line descriptions, an action row (primary create + secondary autodraft, busy labels, post-creation asset-upload note), and an inline `role="alert"` error line (IssueNew pattern) for validation ("choose a tag") and server failures — no native `required` bubble, so the disabled-until-tagged button plus the inline error are the single error channel. No backend or SDK change — autodraft fill, create→detail navigation, and disabled logic are unchanged; assets stay on the detail page.
- **FIXED (issue #50) — empty releases page dedupe:** when the release list is empty the Latest sidebar is not rendered at all and the shared `Empty` callout sits in a centered `max-w-xl` composition, leaving exactly ONE "New release" CTA (the callout's router-link action — the toolbar button and the sidebar's own Empty+CTA go away with their containers; the `refresh` button stays right-aligned). Non-empty state is unchanged: toolbar CTA + list + Latest panel (#35), including the drafts-only case where the sidebar still shows its compact Empty. No backend or SDK change. (The exactly-ONE-CTA rule is superseded by issue #270 below: the toolbar keeps its primary New release on the empty page too — Issues parity, which also pairs its toolbar CTA with the Empty action.)
- **REDESIGNED (issue #270) — releases list + detail match the sibling tabs; asset-upload crash fixed:** (1) crash first — `Release.jsx` `onFile` reset the file input via `e.currentTarget` after the upload await, but Solid nulls `currentTarget` once the handler yields, so the reset threw and the catch toasted `assetCannot set properties of null (setting 'value')`. The handler now captures the input element synchronously before any await (`const input = e.currentTarget`) and resets the captured element in a `finally`; audit found no other `e.currentTarget`-after-`await` handler in the pages (all other event-object uses are synchronous). (2) List goes single-column like Issues (the #35 two-column grid + Latest sidebar is deleted — the sidebar duplicated the first row on most repos): heading + toolbar row mirroring Issues ("Releases" `h2`, right-aligned Refresh + primary New release), All/Drafts/Prereleases pills (`role="group"`, `aria-pressed`, counts) filtering the loaded page client-side via the headless-testable `filterReleases` helper (no new endpoint), name-led divider-separated rows (emerald title link, `latest` chip + `keyAssets` quick-downloads folded inline onto the matching first-page row, `ReleaseBadges`, tag in mono, one-line `excerptBody` excerpt, asset count, `<DateTime>`), "Older" pagination, `Empty` kept (dedicated filtered-empty text when the filter narrows a non-empty page). Drafts stay server-hidden so the Drafts chip reads 0 on most repos — honest, not broken. (3) Detail is name-led: `h1` is the release name (tag fallback), meta row carries tag mono + badges + `<DateTime>` + author + short tag sha; actions group top-right in edit/lifecycle/destructive/navigation order (Edit, Publish primary when draft, Delete danger, Refresh); edit mode is a composer-section form (centered `max-w-2xl` card, Content fieldset with Title/Notes labels + help text, Save primary/Cancel — the IssueNew/PullNew convention). (4) Assets become a `data-table` (icon/name-link/size/short-sha with full-sha title/download/delete, `overflow-x-auto`) under a counted section header; upload is a primary button + drop hint (section-level dragover highlight + drop, same single-file path) with a staged `role="status"` affordance — the page reads + `sha256Hex`-hashes first, then passes the digest so the SDK skips its internal hash, so "Hashing…" (read+hash, the slow part on big files) and "Uploading…" (the POST) stay honest. No new endpoints (assets list already carries name/size/sha256), no new deps. Headless cover: `releases-panel.test.js` gains `filterReleases` + `excerptBody` cases; browser proof: list/detail/upload (incl. the crash scenario) both themes, zero console errors.
...
- **FIXED (issue #133) — app-wide relative date design:** every timestamp renders through one shared `<DateTime>` (`web/src/components/DateTime.jsx`: `<time dateTime>` + hover `title`) backed by the headless-testable `web/src/lib/format.js` helpers. Three tiers — age < 1 day relative only ("just now" < 60 s, "N minute(s) ago" < 60 min, "N hour(s) ago" < 24 h), 1–30 days relative only ("N day(s) ago"; issue #312 moved the old " - {ordinal} of {Month}" suffix into the hover title), 31+ days "{Month} {ordinal}, {Year}"; the hover title is the user's-local wall time "YYYY-MM-DD HH:MM <zone>" (`fmtDateTitle`), never UTC-suffixed, prepended with "{Month} {ordinal} · " for timestamps in the 1–30-day window (e.g. "September 2nd · 2025-09-02 14:03 CDT" — the < 1 day tier needs no calendar date and the 31+ day tier already shows the full date, so both keep the plain wall-time title). Decisions: "month" is a fixed 31-day boundary (whole-day age ≤ 30 middle tier, ≥ 31 absolute — calendar months vary 28–31 days, which would flip the tier on unknowable days); "just now" covers sub-minute ages (the issue's examples start at minutes — a bare "0 minutes ago" must never render); future timestamps clamp to "just now" (clock skew / optimistic rows must never render "-3 minutes ago"); month names are fixed English (deterministic in every browser locale — only the hover title localizes); fallbacks match the old per-page formatters (falsy → "", unparseable → String(input)). Sweeps all previous `fmtDate`/`toLocaleString`/ISO-slice call sites (issues, pulls, commits, releases, checks, comments, WAL, settings incl. webhook deliveries, keys, orgs); `Repo.jsx`/`Org.jsx` per-page formatters deleted. Display-only — no backend, SDK, or data change; no new deps. Headless cover: `web/test/unit/dates.test.js` (tier boundaries, ordinals incl. 11/12/13, future clamp, fallbacks, title shape/zone, #312 1–30-day title prefix with 1/30/31-day boundary pins, `dateTimeAttr` normalization).
...
- **FIXED (issue #214) — refs picker moves under the head pill:** the standalone `refs ▾` dropdown on the right is gone — the left `{branch} @ {sha}` head pill IS the picker trigger (`{label()} ▾`, fed by `head={() => s().head}` so background summary refreshes move the pill without remounting; empty repos keep the static `empty` pill). A move, not a rewrite: the SSE ref stream (50/page, 150 ms debounce, keyed rows), branch/tag kind switch, filter input, and `/{full}/tree/{ref}` switching are byte-identical; the dropdown now anchors `left-0` under the pill (`repo.css` `.ref-drop` follows). Keyboard: native button baseline (Enter/Space toggle, Tab walks controls) plus `aria-haspopup`/`aria-expanded`, Esc closes and refocuses the pill, the filter input autofocuses on open. Clone menu stays on the right; branch/tag counts stay beside the pill. No backend or SDK change; no new deps. Headless cover: `web/test/unit/ref-picker-pill.test.js` (single call site in repo-meta, label shape, right-side picker absence + Clone presence, behavior pins, keyboard contract, left anchor).
- **FIXED (issue #229) — landing hero cleanup: two CTAs only, deep-dive CTA at the page bottom:** the hero stacked competing elements (Browse/Push buttons, a "Configure" link that didn't read as clickable, the compliance paragraph, a How-it-works button). Removed from the hero: the "Configure" link (setup stays in the top nav — `App.jsx` untouched — plus the quickstart auth line), the walgit object-protocol one-liner (verified it still lives on `/how-it-works` per #191 — no duplication, no one-liner added anywhere), and the hero "How it works →" button. Hero is now title + sub + Browse repositories / Push in 30 seconds. The page bottom already carried the deep-dive CTA (quickstart card, secondary `.btn` treatment from #199) — it is now the only one. `LANDING_CTAS` drops the Configure entry. No new deps. Headless cover: `landing.test.js` hero pin rewritten (two CTAs, no Configure/compliance/hero-deep-dive; `/setup` survives only via the quickstart auth line), deep-dive pin tightened to exactly one bottom link with placement assertion, new compliance-lives-on-deep-dive pin. Browser check of `/` both themes is open (no runnable browser in this environment). §2.3 + §2.3.x updated in the same change.
- **FIXED (issue #238) — API link moves to the far-right header cluster:** the `API` entry leaves the primary `site-nav` (remaining order explore → import → keys → setup, unchanged) and renders in the existing `ml-auto` div left of the `NotificationTray` and theme toggle — grouped with the utility chrome, ahead of the fuller footer work that will absorb these links later. Same link treatment: it stays a router `<A>` (identical active-class behavior — there is no explicit nav `.active` visual, so parity is by construction) with the identical text styling via the new `.nav-link` class sharing the `.site-nav a` rule in `ui.css` (dark + light ship together, dark default). Route/href unchanged (`/api`, `Apidocs.jsx`). Narrow widths: the header is a single flex row as before with identical total content, so no new wrap behavior. No new deps. Headless cover: `web/test/unit/nav-api-right.test.js` (nav order/absence, ml-auto placement left of tray + toggle, single `/api` entry + shared styling pin). Browser check of wide + narrow, both themes, is open (no runnable browser in this environment).
- **NEW (issue #235) — repo description + General settings tab:** the repo header (`Repo.jsx` repo-title) renders the summary `description` right of the `owner / repo` title (muted, wrapping below on narrow widths via the existing flex-wrap; omitted entirely when unset — no placeholder). New **General** tab, first in `SETTINGS_GROUP` and the new `DEFAULT_SETTINGS_TAB` (bare `/settings` lands there; `#general` deep-links): a single description input serializing into the WAL-published settings TOML through the existing `repo.settings.get/put` SDK surface (no new endpoint, no new SDK method), with the TOML key scan/set in the headless-testable `web/src/lib/repoDescription.js` (top-level keys only live before the first `[section]` header, so section-nested lookalikes are never read or written). Save invalidates the shared `repo:{full}` summary entry so the header reflects without a full reload; non-admin saves surface the server 403 in the note, matching the other admin-only tabs. Owner-list `<RepoRow>`s explicitly carry no description (names-only listing — per-row summary fetches would be N round trips). No new deps. Headless cover: `web/test/unit/description.test.js` (round-trip, escaping, section isolation, limit) + `settings-nav.test.js` (general-first, default).
- **NEW (issue #240) — pull-only mirrors:** the repo header renders a
  `mirror · pull-only` badge (tooltip: upstream + next sync) plus a
  next-sync/due line from the shared summary (no extra fetch); Settings
  gains a **Mirror** tab after Scheduled tasks (status table, preset picker
  — hourly/8h/daily/weekly/monthly, no freeform cron — Sync-now with
  optional memory-only token + force checkbox and polled status, create
  form on non-mirrors, removal); `/new` gains a mirror-from-URL mode
  (source + preset → create twin → navigate). SDK: `client.mirrors.create`
  + `repo(full).mirror.{get,put,remove,syncNow,syncStatus}` (both lanes via
  the existing rewrites). No new npm deps. Headless cover:
  `web/test/unit/mirror.test.js` (presets, phrasing, validation, tab
  registration) + `web/test/unit/sdk-mirror.test.js` (exact
  endpoints/methods/bodies); the built bundle is verified to carry the
  badge/tab/preset strings and every route serves 200; live-server proof
...
- **FIXED (issue #252) — header pill follows the viewed ref, not always HEAD:** the pill label was built from the shared summary (`s().head` — ref-blind by design, default branch only), so visiting `/tree/{branch}` rendered the branch content with the pill still showing `main @ …`. The pill is now client composition over a `viewed` signal in the Repo shell context (`Repo.jsx`: `viewed`/`setViewed`, null = no ref in view): Tree, Blob, Commits (via a `setViewed` prop on the keyed `CommitList`), Commit (`CommitDetail`, remounted per sha), and CheckDetail publish their resolved `{name, sha}` in a `createEffect` (reusing the resolve step — zero new fetches, and the resolve 5s SWR is what moves the pill when the viewed branch is pushed) and clear it in `onCleanup`, so non-ref tabs (Issues/Pulls/Settings/Checks/Releases) fall back to the summary head with no #214 regression. The head prop is `head={() => pillHead(getViewed(), s().head)}` (context-first, still a getter so background summary refreshes move the pill without remounting the picker); `RefPicker.pick` navigation is unchanged. Sha-addressed views (commit detail, checks/:sha, tree/blob at a raw sha — resolve answers an empty ref there) publish an empty name and the pill shows the short sha honestly, never `sha @ sha` or ` @ sha`. The derivation lives in the headless-testable `web/src/lib/ref-pill.js` (`shortRef` moved there as the single definition, re-exported from `Repo.jsx` so existing `shortRef` importers are untouched; `pillHead` = context-first/fallback, `pillLabel` = `refs` / short-sha / `branch @ shortsha`); the trigger renders `{pillLabel(head())} ▾` with identical styling (dark + light ship together, dark default). No backend, SDK, or API change; no new deps. Headless cover: `web/test/unit/viewed-ref-pill.test.js` (derivation matrix: branch/tag/sha/fallback/placeholder + shell wiring + per-tab publication + non-ref-tab silence) with `ref-picker-pill.test.js` updated to the delegation; `vite build` green. Browser proof (branch navigation, branch deep-link, both themes, zero console errors) is open (shared-daemon network guard blocks private/loopback targets — no private daemon per workspace rules).
- **FIXED (Forgejo #254) — release tag datalist becomes a styled combobox:** the native `<datalist>` on `/releases/new` rendered inconsistently (Chrome shows suggestions only after typing/clicking, Safari/Firefox differ, no styling control, undiscoverable without typing first). The input stays the source of truth with a ▾ trigger opening a `.tag-drop` popover (solid-panel rule + `.scroll-slim`, same treatment as the #115 popovers) listing recent tags in tags-stream order (most recent first); typing filters client-side through the headless-testable `filterTagNames` helper in `web/src/lib/releases.js` (case-insensitive substring, order-preserving, no new fetches — rides the existing `tags:{full}` cache entry). Keyboard mirrors the RefPicker conventions (ArrowUp/Down move with wrap, Enter selects, Esc closes and refocuses the input); a pointer click outside closes it (the LabelPicker/MilestonePicker document-level idiom); selecting fills the input and refocuses it. A11y: input `role="combobox"` with `aria-expanded`/`aria-controls`/`aria-activedescendant`, list `role="listbox"`, rows `role="option"` with `aria-selected`. Empty state stays truthful post-#253 ("No tags yet — create one from a commit page or push one with git" — releases still cannot create tags; the commit view can). No backend, SDK, or API change; no new deps. Headless cover: `web/test/unit/releases-panel.test.js` (blank-query passthrough in stream order, substring/case-insensitivity, no-match empty, non-array empty); `vite build` green. Browser proof (open, filter, select, keyboard, both themes, zero console errors) is open (shared-daemon network guard blocks private/loopback targets — no private daemon per workspace rules).
- **FIXED (issue #275) — wide tables scroll in place, pages never pan at 390px:** the tree and keys tables measured 655/658px and expanded the page (docW 530/439 vs 390) because they rendered in normal flow with no scroll wrapper (only `.blob-cols` had the `overflow-x-auto` treatment). Audit of all 22 `data-table` uses: most were already wrapped (settings tokens/dry-run/webhooks, org, access, releases assets, WAL, keys); three were not and now are — the tree table (`Tree.jsx`), the setup validation-errors table (`Setup.jsx`), and the API-routes table (`Apidocs.jsx`) — each in a plain `overflow-x-auto` div (the established pattern; the `ui.css` nowrap rule inside such wrappers keeps rows single-line, and the wrappers are transparent so dark + light ship together). Narrow-screen collapsing: the tree mode column hides below `sm:` (`hidden sm:table-cell` — icons + the mode lead-char convention from #223 carry the kind on phones), so name + size stay visible with less in-container scrolling; the date column stays (the #211 feature, reachable by the same in-place scroll). Deliberately unwrapped: the `kv` tables (settings describe/effective-values/mirror, API discovery) — their cells wrap text naturally and wrapping them would force single-line via the same nowrap rule; the effective-values value cell and the commit trailer value cell instead gain `break-words` so unbroken strings wrap rather than pan the page. Keys needed no change (already wrapped — predates the issue). No backend, SDK, or API change; no new deps. Headless cover: `node --test` green (548/548) + the structural audit (every non-kv `data-table` inside `overflow-x-auto`); `vite build` green. Browser proof (390px tree/keys scroll-in-place, both themes, zero console errors) is open (shared-daemon network guard blocks private/loopback targets — no private daemon per workspace rules).
- **FIXED (issue #276) — settings nav stacks as full-width chip bars below sm:** the nav shell kept its two sections side by side on phones (`div.flex gap-4`), squeezing the standard bar to ~230px at 390px next to the fixed Danger Zone column. The inner wrapper is now `flex-col` below `sm:` (each section full-width above the content with its own internally-scrolling chip bar), keeps the side-by-side rows at `sm:`–`lg:`, and stays the sticky sidebar at `lg:`+; the Danger section gains `min-w-0` so its bar scrolls instead of forcing width, `nav` gains `min-w-0`, and each `li` is `flex-none` so chips keep content width (column layout unaffected — stretch cross-axis still fills). Native buttons + `aria-current="page"` active styling are unchanged (keyboard flow and active-tab obviousness preserved, dark + light). The nav model (`lib/settingsNav.js`) is untouched — layout-only change. Live-measured page docW at 390px is still > 390 after this change, but the remainder is owned by separate open issues, not the settings shell: the repo tab bar (#274, Settings link reaches R552) and the site-header cluster (#273, R447) — with those two hidden the settings page measures exactly 390 (shell + every tab's content contribute zero page-level overflow). No backend, SDK, or API change; no new deps. Headless cover: `node --test` green; `vite build` green. Browser proof: transient local headless_shell + loopback test server (both killed after; shared daemon untouched) — all 10 tabs × dark/light, zero console errors (see PR).
- **FIXED (issue #277) — pull/review meta rows get a real `.card-meta` rule:** the `card-meta` divs in `Pulls.jsx` (state chip · base←head · author · updated) and `Pull.jsx` ReviewsList (author · badge · date) had zero CSS rules anywhere, so the spans rendered concatenated ("title9 hours ago1 assets") — only accidentally spaced on wide viewports. The rule lands in the LIVE stylesheet `web/src/ui.css` (NOT `web/css/repo.css`, which is dead/unbundled — the issue body's suggested location is corrected here): `flex flex-wrap items-center gap-x-2 gap-y-1 text-xs` muted both themes (`text-zinc-500 dark:text-zinc-400`), matching the Issues/Releases row meta language. Markup gains explicit `{" · "}` separators between the text items (chip/badge stand unseparated, gap-spaced) consistent with the repo-header `state · author · date` row. Scope note: the issue names four pages, but only two `card-meta` uses remain — `Releases.jsx`/`Release.jsx` were redesigned in #270 with Tailwind-inline meta rows and carry no `card-meta`. Audit while here: sibling legacy hooks `card-list` (Pulls/Pull/Checks lists) and `card-title` (Pulls rows) likewise have no rules in the live stylesheet — reported, deliberately unfixed (list spacing + title treatment deserve their own design issue, not a drive-by). No backend, SDK, or API change; no new deps. Headless cover: `web/test/unit/card-meta.test.js` (live-stylesheet pin + dead-css absence, flex/wrap/gap/muted/dark tokens, separator order on both pages); `node --test` green; `vite build` green. Browser proof (release/pull rows mobile + desktop, both themes, zero console errors) is open (shared-daemon network guard blocks private/loopback targets — no private daemon per workspace rules).
- **FIXED (issue #278) — popovers bound to the viewport at 390px:** fixed-width right-anchored panels overflowed a phone viewport — `.clone-body` w-96 measured hanging off at x = -10, the fixed `.tray` (w-96) fully off-screen. Every absolute/fixed panel (`.clone-body`, `.ref-drop`, `.tasks-drop`, `.notif-drop`, `.reaction-drop`, `.label-drop`, `.milestone-drop`, `.close-drop`, `.tag-drop`, `.tray`) is now capped at `max-width: calc(100vw - 1rem)` in the LIVE stylesheet `web/src/ui.css` (NOT `web/css/repo.css`, which is dead/unbundled); right/fixed-anchored panels shrink into view instead of clipping, desktop widths (w-96/w-80/w-64) untouched — the bound only caps. Theme-independent (applies to dark + light alike). Audit fix in the same change: `.milestone-drop` (same shape as `.label-drop`) was missing the #115 opaque-popover treatment — added to the shared solid-panel rule. Deliberately unbounded: `.progress` (`inset-x-0`, already fluid), the notification badge (tiny, not a panel), the clipboard fallback textarea (invisible), the finish-review form (inline, not floating). No backend, SDK, or API change; no new deps. Headless cover: `web/test/unit/popover-viewport.test.js` (live-stylesheet pin + dead-css absence, all-10-panels coverage, desktop-width preservation, 390px fit arithmetic, milestone opacity); `node --test` green (557/557); `vite build` green. Browser proof (390px clone menu + tray fully in view, both themes, zero console errors) is open (shared-daemon network guard blocks private/loopback targets — no private daemon per workspace rules).
- **FIXED (issue #274) — repo tab bar scrolls internally at narrow widths:** the tab row (Code … Settings, measured scrollWidth 515 vs clientWidth 358 at 390px) was a plain flex row with no overflow handling, so trailing tabs cut off mid-letter and the overflow leaked to the page root (docW 531 vs 390). Same treatment as the #273 site-nav strip: the nav gains `max-w-full overflow-x-auto whitespace-nowrap` and scrolls within itself instead of pushing the page sideways; tab links never shrink to unreadability (`.repo-tabs a` → `shrink-0` in the LIVE stylesheet `web/src/ui.css`, whose strip scrollbar is hidden both engines for one stable row height — keyboard Tab still walks the strip and it follows focus). A `createEffect` in `Repo.jsx` keeps the active tab scrolled into view on navigation (`[aria-current="page"]` → `scrollIntoView({block: "nearest", inline: "center"})` — centered in the strip, never moves the page vertically, guarded for non-DOM environments); the truncated peek of the next tab is the touch scroll affordance. `aria-label="repository sections"` and the `aria-current="page"` active style are unchanged (landmark + active-tab obviousness preserved, dark + light). The TABS model (`lib/tabs.js`) is untouched — layout-only change. No backend, SDK, or API change; no new deps. Headless cover: `web/test/unit/repo-tabs-narrow.test.js` (strip-scroll pins, labelled landmark, scrollbar hiding both engines, active-tab scrollIntoView shape incl. block-nearest guard, tab-list invariance, 390px fit arithmetic); `node --test` green (570/570); `vite build` green (compiled CSS carries the three `.repo-tabs` rules). Browser proof (390px tab-strip scroll, active-tab visibility, both themes, zero console errors) is open (shared-daemon network guard blocks private/loopback targets — no private daemon per workspace rules).
- **FIXED (Forgejo #281) — mirror UX gaps (listing badge, waiting state, import creates mirrors):** (1) `<RepoRow>` (`Repos.jsx`, shared by `/explore` and `/:owner`) renders a `pill mirror-badge` chip beside the repo link when the listing row's `mirror` flag is set, with `title`/`aria-label` naming the upstream when `mirror_upstream` is known (`mirror of <url> · pull-only`, else `mirror · pull-only`) — the badge rides the existing detailed-listing payload (same `repos:{owner}` cache key, invalidators untouched), zero new fetches; the flag→text rule is the headless-testable `mirrorRowBadge` in `web/src/lib/mirror.js`. (2) `EmptyRepoGuide` branches on `summary.mirror` (Tree/Blob/Commits all pass the shell summary): mirrors awaiting first sync render `MirrorEmptyGuide` (read-only-mirror copy with the upstream URL, `formatNextSync` first-sync phrasing, failed-last-attempt note, clone command only) and NEVER the push guide — genuinely empty non-mirrors keep the existing guide unchanged. No "Sync now" button: the manual-sync endpoint is admin-only, so a general affordance would promise what the server refuses. (3) `/import` gains an import-once / mirror-continuously mode toggle sharing the source/owner/name/token/dangerous fields: mirror mode swaps the import-only options for the `MIRROR_PRESETS` schedule select, validates through the shared `validateMirrorCreate`, and submits to `repos.mirrors.create` (the #240 endpoint) — the 202 navigates to `/{target}`, where the waiting state from (2) is the progress surface (no SSE attach). Copy keeps the semantic honest everywhere (import = one-shot snapshot, mirror = recurring pull with pushes rejected). No new deps. Headless cover: `web/test/unit/mirror.test.js` (badge presence/label/upstream matrix); `node --test` green; `vite build` green. Browser proof (badges on both listings, waiting state, mirror-from-import, both themes, zero console errors) is open (shared-daemon network guard blocks private/loopback targets — no private daemon per workspace rules).
- **FIXED (Forgejo #283) — explore owner sections ordered by most recent commit:** the owner sections (not just the repo rows) now rank by activity. Each `OwnerSection` reports `ownerActivity(doc)` (max `last_commit_time` over its detailed rows, null when none known) upward in a `createEffect` over the doc it already fetched, and the page orders sections with `orderOwnersByActivity(names, activityMap)` — known times desc, unknown (fetch pending or no commits) last, deterministic name-ascending tiebreak, sliced by the unchanged `MAX_OWNERS` cap after ranking. Zero extra GETs (no per-row commit fetch, no client N+1 — the #247 detailed docs are the only data source); no backend, SDK, or API change (`GET /api/v1/owners` still returns name strings only — a server-side per-owner rollup would remove the first-paint shift but was deliberately out of scope). Trade-offs, documented in code: first paint orders not-yet-loaded owners trailing by name and sections re-rank once as docs land (one visible shift, then settled); all-unknown renders name-ascending (the old reverse-lexicographic proxy is retired from the page, kept as a documented legacy export). `newestFirst` is retained for compatibility with an updated "do not use for explore" comment. No new deps. Headless cover: `web/test/unit/owners.test.js` (`ownerActivity` max/null/doc-or-rows shapes; `orderOwnersByActivity` mixed known/unknown, ties, single-owner, empty, non-array, slice-after-rank); `node --test` green (576/576); `vite build` green. Browser proof (re-ranked explore, both themes, zero console errors) is open (shared-daemon network guard blocks private/loopback targets — no private daemon per workspace rules).
- **Server-side rollup follows the client re-rank (Forgejo #283 follow-up — supersedes the "deliberately out of scope" tail above, appended not rewritten, law 12).** The page now fetches `owners.list({sort:"activity",order:"desc"})` (07 §8 — the frozen string list, server-ordered by the derived per-owner max-commit rollup), so first paint is ordered and ranking covers ALL owners before the `MAX_OWNERS` slice (an active owner past the name cap now surfaces — the structural gap the client rank could never close: unmounted sections never report). The client re-rank (`ownerActivity` reports + `orderOwnersByActivity`) is RETAINED as fallback/enhancement, not removed: it covers missing values (absent catalog, unbackfilled rows) and heals a stale catalog rollup with fresher per-section times, in the same total order so server and client never disagree. New SDK surface `owners.listDetailed()` (per-owner `last_commit_time` rows, same sort query) backs the field for API consumers; `owners.list()` without a query stays the legacy store order (existing callers untouched). Trade-off retired: the "first paint lies, then shifts" transient is gone on catalog-backed instances (only unbackfilled owners trail, then refine once). No new deps. Headless cover: `web/test/unit/sdk-surface.test.js` (list query + listDetailed shapes) + `web/test/unit/sdk-nostore-304.test.js` (both bypass the HTTP cache, issue #200); `owners.test.js` unchanged (fallback semantics pinned there); `vite build` green. Browser proof (server-ordered explore, both themes, zero console errors) is open (shared-daemon network guard blocks private/loopback targets — no private daemon per workspace rules).
- **FIXED (Forgejo #295) — explore shows the top 5 most active owners + the instance owner total.** The page now fetches `owners.listDetailed({sort:"activity",order:"desc"})` (the #283 rows, server-ranked) instead of the plain string list and runs the new pure `activeOwnerNames` filter (null `last_commit_time` dropped — owners with no commits never mount) BEFORE the `MAX_OWNERS` slice, which drops 50 → 5. Fewer than 5 active owners renders fewer sections (no filler); the client re-rank (`orderOwnersByActivity`, still gated on `hasKnownActivity`) now refines the shown sections only. The "showing newest N" line dies with the 50-cap: overflow past 5 reads "showing top 5 of A active owners" (active count only), and with ≤5 active there is no line at all — the page is just the top list. The intro card carries the instance owner total (the payload's uncapped row count — "Home to N owners"; empty instances keep "no repositories yet", owners-but-no-commits get "no commit activity yet"). Cold-load cost drops from ~51 to 6 GETs (1 listing + 5 sections, pinned by a fetch-counting headless test). Deliberate gap, NOT freelanced: there is NO instance repo total on the page — no endpoint serves one (`owners/detailed` rows carry no repo counts, there is no stats endpoint, and walking every owner's listing would be the N-fetch antipattern the page avoids), so the page states the owner total only rather than printing a capped sum as a total. That count needs a backend rail (e.g. `repo_count` on the `owners/detailed` rows or a dedicated stats surface, ghost-filtered like `liveRepos`) — reported on the issue as follow-up. No backend, SDK, or API change; no new deps. Headless cover: `web/test/unit/owners.test.js` (`activeOwnerNames` order/filter/edge shapes, active-filter→top-5→uncapped-total composition, fewer-than-5, cold-load fetch budget); `node --test` green; `vite build` green. Browser proof (top-5 explore + totals line, both themes, zero console errors) is open (shared-daemon network guard blocks private/loopback targets — no private daemon per workspace rules).
- **FIXED (Forgejo #301) — Code tab rows show their own last-commit date.** The date column rendered one value on every row: the HEAD commit's date from a `commits?ref=<sha>&n=1` fetch stamped across the table (the #211 proxy — honest then: per-row truth cost N log walks). The tree payload now carries `commit_sha`/`commit_time` per entry (07 §9.4, one batched server-side walk per listing, capped by `server.max_tree_log`), so each row renders `<DateTime value={e.commit_time} fallback="—">` — the #133 component, whose falsy path already renders the fallback with no `<time>`. The proxy is removed honestly, not hidden: the extra `commits?n=1` fetch and the `latestActivity` import are gone (zero extra round trips — the dates ride the tree payload; submodules and capped-walk misses render the em-dash, never the HEAD date). SDK `TreeEntry` typedef gains the optional fields. No new deps. Headless cover: backend table tests (assignment, renames-as-add/delete, cap, submodules, empty) + real-git integration in `internal/api/tree_dates_test.go`; `node --test` green; `vite build` green. Browser proof (per-row dates differ on a lived-in repo, both themes, zero console errors) is open (shared-daemon network guard blocks private/loopback targets — no private daemon per workspace rules).
- **FIXED (Forgejo #307) — explore shows the instance repo total.** The intro card reads "Home to N owners and M repositories" (pluralized): N stays the payload's uncapped row count (#295), M is the new pure `instanceRepoTotal` helper (sum of the served `repo_count` fields over the uncapped `owners/detailed` payload — never the top-5 slice, never a per-owner listing walk). This closes the deliberate #295 gap (the "no endpoint serves one" NOTE in `Owners.jsx` is now a CLOSED pointer to the rail). No new fetches (same listing GET), no new deps. Headless cover: `web/test/unit/owners.test.js` (`instanceRepoTotal` uncapped-sum, capped-sum-is-not-the-total, missing-count/legacy tolerance); `node --test` green; `vite build` green. Browser proof open (shared-daemon loopback guard).
