...
- **Order**: owners newest-first, repos newest-first. Ordering key: reverse of server order — the
  bucket stores no creation timestamps for owners or repos and the endpoints return store-sorted
  (ascending) names, so reverse-lexicographic is the closest deterministic newest-first proxy. The
  single function to replace when the backend carries creation times is `newestFirst` in
  `web/src/lib/owners.js`.
