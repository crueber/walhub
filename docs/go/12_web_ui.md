...
- **Caps**: `MAX_OWNERS` 50 owners, `MAX_REPOS_PER_OWNER` 10 repos per section (constants in
  ...
  blocks the rest).
- **Styling** (issue #137): the intro keeps its `.card`, but owner sections are flat —
  no cards-in-cards. Sections sit in a `divide-y divide-zinc-200 dark:divide-zinc-800`
  stack (dividers, not nested containers) with a quieter `text-sm` owner heading; shared
  `.muted`/emerald-link classes — correct in dark (default) and light with no page-specific
  colors. Links stay plain anchors (keyboard flow unchanged).
