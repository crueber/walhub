...
- **Order**: owner sections most-recent-commit-first (Forgejo #283 — each
  section reports `ownerActivity` over the detailed doc it already fetches,
  the page re-ranks via `orderOwnersByActivity`; unknown/pending sorts last,
  name tiebreak), repos most-recent-commit-first (Forgejo #247). The listing
