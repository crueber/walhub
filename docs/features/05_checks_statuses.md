...
- **Checks-subsystem audit verdict: live and fully wired, not dead infrastructure (Forgejo #504).** A static investigation of the current tree confirmed every layer is composed, routed, and consumed on both the write and read paths — the dead-infrastructure hypothesis is rejected. Write path: external CI authenticates with `wct_` tokens claimed by the `AuthService.ExtraCredential` hook (`cmd/walhub/checks.go:chainChecks`, `checks.ClaimToken`/`ParseCIToken` → unprivileged `ci:<id>` principal, secret verified handler-side per repo) and reports via `POST /{o}/{r}/api/checks/statuses/{sha}`; discovery rides `checks.ExposedTemplates` registered from composition (`newChecksService`, `api.RegisterExposed`). Composition: `cmd/walhub/collab.go` wires `newChecksService` and chains the handler onto the server; the notify fanout subscribes to checks events (05 §8). Read path: the merge task consults the stored combined view through pulls' `ChecksGate` seam (`internal/pulls/checks.go`, fail-closed — a `require_checks` rule with no checks backend refuses the merge) at the merge.go step-4 call site; the checks table page, repo Checks tab, CI-tokens settings tab, PR combined view, commit pills, and the `checks` SDK submodule all consume the surface. By design nothing inside walhub *produces* statuses — they arrive exclusively from external CI via the `wct_` token API (05 is the consumer half of an external-CI contract, the Forgejo-statuses + Woodpecker shape). No dead code found; no code changes made (static investigation only).
