// visibility.go — visibility listing filters (Forgejo #345, modes #374).
//
...
// admin flag skip the probes entirely (CheckRead early-allows them,
...
// host admin caller → the registry list unfiltered (+0 store trips).
...
// readable by p. Nil Access or a host admin caller → the registry
...
// Nil Access or a host admin caller → every owner's full list.
...
// probes: no gate wired (legacy), or a host admin principal (the
// require_read hook early-allows admins without consulting access.json).
// Host WRITE alone bypasses nothing (Forgejo #374, the #347 direction
// extended to reads): writers pay one CheckRead probe per candidate repo
// exactly like ordinary users, so a private name can never leak to them
// through a listing they could not open.
func (e *Env) unfiltered(p auth.Principal) bool {
	if e.Access == nil {
		return true
	}
	return !p.Anonymous && p.Admin
}
