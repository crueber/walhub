          ...
          return (
            <>
              ...
              <div class="blob-head mb-2 flex flex-wrap items-baseline gap-2">
                ...
                <span class="muted tabular text-xs">{b().size ?? "?"} bytes · {String(b().sha).slice(0, 12)}</span>
                <Show when={rawHref()}>
                ...
                <Switch>
                  <Match when={b().too_large}>
                    <p class="muted italic p-6 text-center">
                      This file is too large to render ({b().size ?? "?"} bytes; the render cap is 2 MiB).
                      Fetch it raw from the API.
                    </p>
                  </Match>
                  <Match when={b().binary}>
                    <p class="muted italic p-6 text-center">binary file, {b().size ?? "?"} bytes</p>
                  </Match>
                  <Match when={isMd()}>
                    ...
                    <Show
                      ...
                      fallback={
                        <pre class="code-view m-0 rounded-none border-0">
                  ...
                  <Match when={true}>
                    <div class="blob-cols flex overflow-x-auto">
                      ...
                      <pre class="code-view m-0 rounded-none border-0 px-3.5 py-3">
                        <code innerHTML={highlight(b().contents ?? "", lang())} />
                      </pre>
                    </div>
                  </Match>
