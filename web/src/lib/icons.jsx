...
// viewBox, aria-hidden, class) with its glyph children inline, transcribed
// verbatim from the provided 1em currentColor files (issue comments 4783-4792 — the
...
// height="1em"). The eleventh entry is the create-button plus (Forgejo #466):
// no plus glyph was provided with #465, so it is a minimal inline stroke
// drawn here as its own complete svg (16-unit viewBox, currentColor
// paint, no color literals) rather than a one-off <svg> at the call site.
// The twelfth entry is the issue-list comment bubble (Forgejo #481),
// transcribed verbatim from the issue-provided 16-unit file.
// The thirteenth and fourteenth entries are the milestone open/done pair
...
// The fifteenth entry is the label/tag icon (Forgejo #485), transcribed
// verbatim from the issue-provided 24-unit file (viewBox kept as-is,
// currentColor stroke paint, no color literals).
//
// Forgejo #491: entries MUST stay per-entry factories returning a complete
// svg — never module-level JSX values shared across renders, never one
// shared outer shape with the glyph composed into it at runtime. Module-level
// JSX evaluates ONCE (a single DOM node): rendering <Icon name="issue-comment" />
// once per <For> row then moves that SAME node from row to row, so every row
// but the last mounts an empty svg (the #481 shape did exactly this with one
// shared svg plus an imperative body insert). A factory body re-executes on
// every Icon render, so every row mints fresh nodes — the same reason the
// per-entry inline svgs in components/Empty.jsx never share nodes. Icon
// renders the factory result directly: no shared shape, no runtime
// composition into a cached template.
...
// literals anywhere in this layer — every glyph paints via fill="currentColor"
...
// are the point: the 1em width/height on each entry's own <svg> scales every
 ...
 * The entry factory runs on every render, so each Icon mints its own svg
 * nodes (sharing one node across rows would leave all but the last row
 * empty — see the no-sharing note in the header above). The icon is decorative — every svg carries aria-hidden="true"
 * and the control's own title/aria-label/aria-pressed stays the accessible
 * signal. Unknown names render nothing (never a broken glyph).
 */
export default function Icon(props) {
  const make = () => ICONS[props.name];
  const cls = () => (props.class ? `icon ${props.class}` : "icon");
  // NOTE: a non-keyed <Show> hands function children an accessor, not the
  // value — entry() unwraps it to the factory, and invoking that factory
  // here, per render, is what mints fresh nodes for every row (#491).
  return <Show when={make()}>{(entry) => entry()(cls())}</Show>;
}
